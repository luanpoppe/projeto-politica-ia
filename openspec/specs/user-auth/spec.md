...
Autenticação e identidade de cidadãos no backend: cadastro, login, refresh com rotação de tokens e logout global. Base para funcionalidades cívicas futuras, com persistência separada (PostgreSQL para identidade, Redis para sessões) e arquitetura Clean Architecture + DDD.
