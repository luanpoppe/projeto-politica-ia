...
Autenticação no frontend Next.js: cadastro, login, persistência e renovação de tokens, logout e área autenticada mínima, integrados à API `user-auth` do backend.
...
- **AND** exibe feedback de sucesso (toast)
- **AND** redireciona para `/login` sem autenticar automaticamente
